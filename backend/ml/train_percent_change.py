...
EXCLUDE_DEFAULT = [
    'SERIAL NO', 'STOCK NAME', 'SYMBOL', 'FIXED SYMBOL'
]
...
def parse_args():
    ...
    p.add_argument('--date-col', default=None, help='Date column name (auto-detect if omitted)')
    p.add_argument('--target-col', default='PERCENT CHANGE', help='Target column to predict')
    p.add_argument('--exclude', nargs='*', default=EXCLUDE_DEFAULT, help='Columns to exclude from features')
    p.add_argument('--id-cols', nargs='*', default=[], help='Additional identifier columns to drop from features')
    p.add_argument('--lags', nargs='*', type=int, default=[1, 3, 5], help='Lag periods for numeric features')
    p.add_argument('--rollings', nargs='*', type=int, default=[3, 5, 10], help='Rolling windows for stats')
    p.add_argument('--outdir', default='models', help='Output directory')
    p.add_argument('--test-size', type=float, default=0.2, help='Proportion for time-based validation split (0-1)')
    return p.parse_args()
...
def build_lag_rolling(df: pd.DataFrame, num_cols: List[str], group_cols: List[str], lags: List[int], rollings: List[int]) -> pd.DataFrame:
...
def train_models(X_train, y_train, X_val, y_val, feature_names: List[str]) -> Tuple[Pipeline, dict, pd.DataFrame]:
    results = []
    ...
    rf_base = Pipeline([
        ('prep', preproc_tree),
        ('model', RandomForestRegressor(random_state=42, n_estimators=300, max_depth=None))
    ])
    ...
    if HAS_XGB:
        xgb = Pipeline([
            ('prep', preproc_tree),
            ('model', XGBRegressor(random_state=42, n_estimators=400, max_depth=6, learning_rate=0.05, subsample=0.9, colsample_bytree=0.9, tree_method='hist'))
        ])
    ...
    best_model = lin_pipe if best_name == 'LinearRegression' else rf_base if best_name == 'RandomForest' else best_xgb
    ...
    try:
        ...
        if best_name in ['RandomForest', 'XGBoost']:
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                feat_imp = pd.DataFrame({'feature': full_cols, 'importance': vals}).sort_values('importance', ascending=False)
    except Exception:
        pass

    return best_model, {r['model']: {k: float(r[k]) for k in ['MAE','RMSE','R2']} for _, r in df_results.iterrows()}, df_results
...
def main():
    ...
    exclude = set([c for c in args.exclude if c in df.columns] + [c for c in args.id_cols if c in df.columns] + ['target_next'])
    ...
    numeric_cols = [c for c in df.select_dtypes(include=[np.number]).columns if c not in exclude and c != 'target_next']
    ...
    # Final feature set
    feature_cols = [c for c in df.columns if c not in exclude and c not in [args.target_col, 'target_next']]
    ...
    best_model, metrics_map, df_results = train_models(X_train, y_train, X_val, y_val, feature_cols)
    ...
    print(json.dumps({'ok': True, 'best_model': report['best_model_path'], 'metrics': metrics_map}))
