...
def detect_swing_highs_lows(
    df: pd.DataFrame, swing_len: int = 5, suffix: str = ""
) -> pd.DataFrame:
    """
    ...
    df[sh_col] = (
        df["High"] == df["High"].rolling(window, center=True, min_periods=window).max()
    )
    df[sl_col] = (
        df["Low"] == df["Low"].rolling(window, center=True, min_periods=window).min()
    )
    ...
    for idx in df.index:
        if df.loc[idx, sh_col]:
            ...
            if prev_sh is None or curr_sh > prev_sh:
            ...
            events.append(
                {
                    ...
                    "leg_start_idx": int(last_sl_idx)
                    if last_sl_idx is not None
                    else None,
                    "label": str(df.loc[last_sh_idx, label_sh_col])
            ...
            events.append(
                {
                    ...
                    "leg_start_idx": int(last_sh_idx)
                    if last_sh_idx is not None
                    else None,
                    "label": str(df.loc[last_sl_idx, label_sl_col])
                    if last_sl_idx is not None
                    else "",
                }
            )
...
def detect_equal_highs_lows(
    ...
    for i in range(1, len(swing_highs)):
        _prev_date, prev_high, prev_idx = swing_highs[i - 1]
        curr_date, curr_high, curr_idx = swing_highs[i]
        if (curr_idx - prev_idx) >= bars_confirm and abs(curr_high - prev_high) <= threshold:
            eqh.append(
                {
                    "date": curr_date,
    ...
    for i in range(1, len(swing_lows)):
        _prev_date, prev_low, prev_idx = swing_lows[i - 1]
        curr_date, curr_low, curr_idx = swing_lows[i]
        if (curr_idx - prev_idx) >= bars_confirm and abs(curr_low - prev_low) <= threshold:
            eql.append(
                {
                    "date": curr_date,
...
def compute_premium_discount(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if swing_high is None:
        recent = df.tail(252)
        swing_high = (
            float(recent["High"].max()) if not recent.empty else float(df["High"].max())
        )

    if swing_low is None:
        recent = df.tail(252)
        swing_low = (
            float(recent["Low"].min()) if not recent.empty else float(df["Low"].min())
        )
...
def compute_smc_score(
    ...
    # 1. Active bullish OB
    bull_obs = [
        o for o in active_obs if o["direction"] == "bullish" and o["status"] == "Active"
    ]
    if bull_obs:
        score += 25
...
def run_full_smc_analysis(
    ...
    # Active OBs
    active_bull_swing = [
        o for o in swing_obs if o["direction"] == "bullish" and o["status"] == "Active"
    ]
    active_bear_swing = [
        o for o in swing_obs if o["direction"] == "bearish" and o["status"] == "Active"
    ]
    active_bull_internal = [
        o
        for o in internal_obs
        if o["direction"] == "bullish" and o["status"] == "Active"
    ]
    active_bear_internal = [
        o
        for o in internal_obs
        if o["direction"] == "bearish" and o["status"] == "Active"
    ]
    ...
    if active_bull_swing:
        nearest_bull = min(
            active_bull_swing, key=lambda o: abs(ltp - o.get("zone_mid", ltp))
        )
    elif active_bull_internal:
        nearest_bull = min(
            active_bull_internal, key=lambda o: abs(ltp - o.get("zone_mid", ltp))
        )
    ...
    if not last_bull_ev:
        bullish_events_internal = [
            e for e in internal_events if e["direction"] == "bullish"
        ]
    ...
    return {
        ...
        "internal_structure": {
            "last_bullish_event": (
                internal_events[-1]["event"] if internal_events else None
            ),
            "last_bullish_date": (
                internal_events[-1]["date"] if internal_events else None
            ),
