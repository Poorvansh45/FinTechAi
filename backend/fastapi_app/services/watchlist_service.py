...
class WatchlistService:
    ...
    async def get_user_watchlists(self, user_id: str) -> list[dict[str, Any]]:
        cursor = self.watchlists_coll.find({"created_by": user_id}).sort(
            "created_at", -1
        )
        watchlists = []
    ...
    async def rename_watchlist(
        self, watchlist_id: str, new_name: str, user_id: str
    ) -> None:
        try:
    ...
    async def delete_watchlist(self, watchlist_id: str, user_id: str) -> None:
        ...
        async for stock_doc in stocks_cursor:
            ...
            final_price = (
                quote.price
                if quote.available and quote.price
                else stock_doc.get("added_price", 0)
            )
    ...
    async def add_stock(
        self, watchlist_id: str, data: WatchlistStockCreate, user_id: str
    ) -> WatchlistStockInDB:
        try:
            wl_id = ObjectId(watchlist_id)
        except Exception:
        ...
        existing = await self.stocks_coll.find_one(
            {"watchlist_id": wl_id, "symbol": data.symbol}
        )
        if existing:
            raise HTTPException(
                400, f"Stock {data.symbol} is already in this watchlist"
            )
    ...
    async def remove_stock(self, watchlist_id: str, symbol: str, user_id: str) -> None:
        ...
        stock_doc = await self.stocks_coll.find_one(
            {"watchlist_id": wl_id, "symbol": symbol}
        )
        if not stock_doc:
    ...
    async def get_watchlist_details(
        ...
        for s in stocks:
            ...
            current_price = (
                quote.price
                if quote and quote.available and quote.price
                else s.added_price
            )
            ...
            if not historical_prices.empty and s.symbol in historical_prices.columns:
                ...
                post_add_prices = stock_prices[
                    stock_prices.index.tz_localize(None) >= s.added_date
                ]
                ...
                if "^NSEI" in historical_prices.columns:
                    ...
                    nifty_post_add = nifty_prices[
                        nifty_prices.index.tz_localize(None) >= s.added_date
                    ]
                ...
                if not post_add_prices.empty and s.added_price > 0:
                    ...
                    for days, label in [
                        ...
                        if len(post_add_prices) > days:
                            ...
                            comp_returns[label] = (
                                (target_price - s.added_price) / s.added_price
                            ) * 100
        ...
        if not historical_prices.empty:
            portfolio_cols = [
                s.symbol for s in stocks if s.symbol in historical_prices.columns
            ]
            if portfolio_cols:
                daily_returns_df = (
                    historical_prices[portfolio_cols].pct_change().dropna(how="all")
                )
                if not daily_returns_df.empty:
                    port_returns = daily_returns_df.mean(axis=1)
                    if len(port_returns) > 1:
                        overall_volatility = round(
                            float(port_returns.std() * (252**0.5) * 100), 2
                        )

        overall_drawdown = (
            sum(s["current_drawdown_pct"] for s in enriched_stocks) / total_stocks
            if total_stocks > 0
            else 0.0
        )
        overall_alpha = (
            sum(s["alpha_vs_nifty"] for s in enriched_stocks) / total_stocks
            if total_stocks > 0
            else 0.0
        )
        ...
        stats = {
            ...
            "win_rate_pct": (win_count / total_stocks * 100)
            if total_stocks > 0
            else 0.0,
    ...
    async def get_watchlist_performance_by_source(
        self, watchlist_id: str, user_id: str
    ) -> dict[str, Any]:
    ...
    async def get_watchlist_leaderboard(self, user_id: str) -> list[dict[str, Any]]:
        ...
        cursor = self.watchlists_coll.find(
            {"is_archived": {"$ne": True}, "created_by": user_id}
        )
                ...
                result.append(
                    {
                        ...
                        "avg_return_pct": round(
                            data["total_return"] / data["count"], 2
                        ),
