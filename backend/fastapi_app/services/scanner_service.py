...
class ScannerService:
    ...
    async def get_technical_screener(
        ...
        r(
            query,
            "indicators.ema_50_dist_pct",
            filters.get("ema50_dist_min"),
            filters.get("ema50_dist_max"),
        )
        r(
            query,
            "indicators.ema_200_dist_pct",
            filters.get("ema200_dist_min"),
            filters.get("ema200_dist_max"),
        )
        ...
        cursor = (
            self.screener.find(query, {"_id": 0}).sort(sort_key, sort_dir).limit(limit)
        )
    ...
    async def get_volume_surges(
        ...
        r(
            query,
            "day_return_pct",
            filters.get("day_return_min"),
            filters.get("day_return_max"),
        )
        r(
            query,
            "surge_stats.positive_surge_pct",
            filters.get("positive_surge_pct_min"),
        )
        ...
        cursor = (
            self.surge_cache.find(query, projection)
            .sort("volume_ratio", -1)
            .limit(limit)
        )
        return await cursor.to_list(length=limit)
    ...
    async def get_fvg_stocks(
        ...
        r(
            query,
            "nearest_fvg_dist_pct",
            filters.get("distance_fvg_min"),
            filters.get("distance_fvg_max"),
        )
        ...
        r(
            query,
            "distance_high_pct",
            filters.get("distance_high_min"),
            filters.get("distance_high_max"),
        )
        r(
            query,
            "distance_low_pct",
            filters.get("distance_low_min"),
            filters.get("distance_low_max"),
        )
        ...
        cursor = (
            col.find(query, {"_id": 0}).sort([("nearest_fvg_dist_pct", 1)]).limit(limit)
        )
        return await cursor.to_list(length=limit)
    ...
    async def get_momentum_stocks(
        ...
        if count == 0:
            ...
            cursor = (
                self.screener.find(query_fb, {"_id": 0})
                .sort("indicators.rsi_14", -1)
                .limit(limit)
            )
            return await cursor.to_list(length=limit)
