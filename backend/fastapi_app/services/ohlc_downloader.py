...
def get_downloader_paths() -> dict[str, str]:
    ...
    upstox_csv = os.path.realpath(
        os.path.join(scripts_dir, "upstox_nse_stock_list.csv")
    )
    ...
    return {"symbols_csv": symbols_csv, "output_csv": output_csv}
...
def refresh_in_memory_cache() -> None:
    ...
    if not os.path.exists(output_csv):
        _IN_MEMORY_STOCK_CACHE = {}
        log.warning(
            f"Stock_Data.csv not found at {output_csv}. In-memory cache is empty."
        )
        return
    ...
    try:
        ...
        log.info(
            f"Loaded {len(_IN_MEMORY_STOCK_CACHE)} symbols into memory cache in {time.time() - t0:.2f}s."
        )
...
def fetch_yfinance_incremental(symbol, start_dt, end_dt):
    yf_symbol = (
        symbol if symbol.endswith(".NS") or symbol == "^NSEI" else f"{symbol}.NS"
    )
    try:
...
def safe_get_upstox_candles(instrument_key: str, from_date: str, to_date: str) -> Any:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if resp.status_code == 429:
            ...
            log.warning(
                f"⚠️ Upstox Rate limit → retry {attempt}/{MAX_RETRIES} in {sleep_time}s"
            )
...
def fetch_upstox_range(instrument_key: str | None, start_dt, end_dt) -> pd.DataFrame:
...
def _resolve_instrument_key(row) -> str | None:
...
async def download_incremental_ohlc() -> str:
    ...
    symbols_df["Symbol"] = (
        symbols_df["trading_symbol"].astype(str).apply(normalize_symbol)
    )
    ...
    if _use_mongo():
        ...
        log.info(
            f"Mongo backend: {len(last_date_map_mongo)} symbols already stored "
        ...
        try:
            ...
            log.error(
                f"Failed to read existing Stock_Data.csv: {e}. Rebuilding from scratch."
            )
            existing_df = pd.DataFrame(columns=EMPTY_COLS)
    else:
        last_date_map_mongo = None
        log.info(
            "No existing Stock_Data.csv found. Will perform first-time setup (5yr history)."
        )
    ...
    if last_date_map_mongo is not None:
        ...
        last_date_map = existing_df.groupby("symbol")["date"].max().to_dict()
    ...
    if new_records:
        ...
        log.info(
            f"✅ Stock_Data.csv updated successfully. Total records on disk: {len(combined)}"
        )
    else:
        log.info("No new EOD candles were fetched (all tickers up to date).")
        ...
        if not existing_df.empty:
            ...
            log.info(
                f"🧹 Standardized and saved existing database file. Rows: {len(cleaned_existing)}"
            )
