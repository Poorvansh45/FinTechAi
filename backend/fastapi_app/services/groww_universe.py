...
class GrowwUniverseService:
    def __init__(self, username: str = None, password: str = None, pin: str = None):
    ...
    def _init_api(self):
        ...
        try:
            ...
            if self.username and self.password and self.pin:
                ...
                token = GrowwAPI.get_access_token(self.username, self.password, self.pin)
            else:
    ...
    def fetch_master_universe(self) -> List[Dict[str, Any]]:
        ...
        if not self.api:
            ...
            return [
                {"symbol": "RELIANCE", "company_name": "Reliance Industries", "isin": "INE002A01018"},
                {"symbol": "TCS", "company_name": "Tata Consultancy Services", "isin": "INE467B01029"},
                {"symbol": "HDFCBANK", "company_name": "HDFC Bank", "isin": "INE040A01034"},
        ...
        try:
            ...
            log.info(f"Filtered {len(filtered_universe)} NSE CASH EQ stocks from universe.")
