...
def generate_rebalance_suggestions(
    ...
    sector_exposure: Optional[list[dict]] = None,
    ...
    if sorted_holdings and sorted_holdings[0][1] > 35:
        ...
        if not any(s["ticker"] == ticker and s["category"] == "drift" for s in suggestions):
            suggestions.append({
                "ticker": ticker,
                "action": "trim",
    ...
    if health_data and health_data.get("score", 100) < 50:
        if health_data["breakdown"]["diversification"] < 40:
            suggestions.append({
                "ticker": "Portfolio",
                "action": "add",
                "current_pct": 0,
