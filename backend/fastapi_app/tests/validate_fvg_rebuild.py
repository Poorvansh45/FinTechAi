...
def run_validation():
    ...
    for sym in sample_symbols:
        ...
        print(
            f"Symbol {sym:12} | Ref count: {ref_count:3} | New count: {new_count:3} | Count Match: {'✓' if count_match else 'X'} | Zones Match: {'✓' if zones_match else 'X'}"
        )
    ...
    if count_accuracy >= 95.0 and zones_accuracy >= 95.0:
        print(
            "✓ SUCCESS: Refactored FVG scanner matches Jupyter Notebook logic perfectly!"
        )
        sys.exit(0)
    else:
