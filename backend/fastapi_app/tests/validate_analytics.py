...
try:
    ...
    record("analytics.risk_engine", FAIL, str(e))
    sys.exit(1)
...
try:
    ...
    record("analytics.diversification", FAIL, str(e))
    sys.exit(1)
...
try:
    ...
    record("analytics.health_score", FAIL, str(e))
    sys.exit(1)
...
try:
    ...
    record("analytics.sector_analysis", FAIL, str(e))
    sys.exit(1)
...
try:
    ...
    record("analytics.rebalancer", FAIL, str(e))
    sys.exit(1)
...
try:
    ...
    record("portfolio.calculator", FAIL, str(e))
    sys.exit(1)
...
check(
    "volatility realistic 5-50%", 0.05 < vol < 0.50, f"{vol * 100:.1f}%", warn_only=True
)
...
check(
    "sortino >= sharpe - 0.5 (less penalty)",
    sortino >= sharpe - 0.5,
...
check(
    "VaR 95% is negative", var_stats["var_95"] < 0, f"var95={var_stats['var_95']:.5f}"
)
check(
    "VaR 99% <= VaR 95%",
    var_stats["var_99"] <= var_stats["var_95"],
    f"var99={var_stats['var_99']:.5f}",
)
check(
    "VaR 95% in plausible range",
    -0.20 < var_stats["var_95"] < 0,
...
risk = estimate_risk_level(
    volatility=0.22, concentration_score=55, sector_concentration=38.0
)
check(
    "risk level keys present",
    all(k in risk for k in ["level", "score", "color", "factors"]),
)
check("risk score in [0, 100]", 0 <= risk["score"] <= 100, f"score={risk['score']}")
check(
    "risk level valid string", risk["level"] in ["Low", "Moderate", "Elevated", "High"]
)
check("risk color is hex", risk["color"].startswith("#"))

risk_high = estimate_risk_level(
    volatility=0.45, concentration_score=90, sector_concentration=70
)
risk_low = estimate_risk_level(
    volatility=0.05, concentration_score=10, sector_concentration=15
)
check(
    "high inputs -> higher risk score",
    risk_high["score"] > risk_low["score"],
...
price_s = pd.Series(
    np.cumprod(1 + np.random.normal(0.0006, 0.015, 504)) * 100, index=dates
)
...
check(
    "single stock diversification = 0",
...
check(
    "effective stocks = 1 for single holding",
    0.9 < compute_effective_number_of_stocks([100.0]) < 1.1,
)

conc = compute_concentration_score([40, 30, 20, 10])
check(
    "concentration has required keys",
    all(k in conc for k in ["hhi", "effective_stocks", "top_holding_pct", "score"]),
)
check(
    "top_holding_pct = 40",
    abs(conc["top_holding_pct"] - 40.0) < 0.5,
    f"{conc['top_holding_pct']}",
)
check(
    "top3_holding_pct = 90",
    abs(conc["top3_holding_pct"] - 90.0) < 0.5,
    f"{conc['top3_holding_pct']}",
)
check("concentration score in [0,100]", 0 <= conc["score"] <= 100, f"{conc['score']}")
check(
    "concentration level valid",
    conc["concentration_level"] in ["low", "moderate", "high"],
)
...
check(
    "strong > mid > weak",
    h_strong["score"] > h_mid["score"] > h_weak["score"],
    f"{h_strong['score']} > {h_mid['score']} > {h_weak['score']}",
)
check("breakdown has 4 keys", len(h_strong["breakdown"]) == 4)
check(
    "breakdown values in [0,100]",
    all(0 <= v <= 100 for v in h_strong["breakdown"].values()),
)
...
check(
    "total_pnl accurate",
    abs(totals["total_pnl"] - expected_pnl) < 1.0,
    f"expected={expected_pnl} got={totals['total_pnl']}",
)
check(
    "allocations sum to 100%", abs(sum(h["allocation"] for h in enriched) - 100.0) < 0.5
)
check(
    "total_value matches sum",
    abs(totals["total_value"] - sum(h["value"] for h in enriched)) < 0.01,
)
...
sugs = generate_rebalance_suggestions(
    current_weights=current_w, sector_exposure=exposure
)
...
check(
    "equal-weight has 0 drift suggestions",
...
check(
    ...
    all(
        p_map[priorities[i]] <= p_map[priorities[i + 1]]
...
check(
    "volatility empty weights = 0.0",
    compute_portfolio_volatility(np.array([]), np.array([[]])) == 0.0,
)
check(
    "max_drawdown empty = 0.0",
    compute_max_drawdown(pd.DataFrame(), np.array([])) == 0.0,
)
check(
    "var empty = defaults",
    compute_var(pd.DataFrame(), np.array([]))
    == {"var_95": 0.0, "var_99": 0.0, "var_95_annual": 0.0, "var_99_annual": 0.0},
)
check(
    "beta short series = 1.0", compute_beta(np.array([0.01]), np.array([0.01])) == 1.0
)
