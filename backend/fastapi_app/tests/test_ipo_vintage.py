...
def test_trigger_entry_stop_and_risk_are_correct():
    ...
    assert out["breakout_strength_pct"] == round(
        (125.0 - 120.0) / 120.0 * 100.0, 2
    )  # 4.17
...
def test_stop_and_horizon_on_the_same_bar_stop_wins():
    ...
    assert out["horizons"]["h7"]["return_pct"] == round(
        (80.0 - 125.0) / 125.0 * 100.0, 2
    )
...
def test_live_window_boundary_is_inclusive():
    for offset, expected in (
        (LIVE_WINDOW_SESSIONS, "live"),
        (LIVE_WINDOW_SESSIONS + 1, "expired"),
    ):
        rows = [(90, 120, 80, 100, 50_000), (120, 130, 118, 125, 40_000)]
...
def test_tight_risk_outscores_wide_risk_at_equal_breakout_strength():
    tight, tight_subs = score_ipo_vintage(
        breakout_strength_pct=4.0, risk_pct=5.0, volume_ratio=1.5
    )
    wide, wide_subs = score_ipo_vintage(
        breakout_strength_pct=4.0, risk_pct=24.0, volume_ratio=1.5
    )
    assert tight > wide
...
def test_scorer_shape_and_bounds():
    conf, breakdown = score_ipo_vintage(
        breakout_strength_pct=8.0, risk_pct=5.0, volume_ratio=2.5
    )
