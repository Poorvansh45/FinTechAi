...
def test_redesign():
    ...
    if df.empty:
        print(f"[FAIL] BLUECHIP data is empty in Stock_Data.csv.")
        sys.exit(1)
    ...
    if len(fvg_data['fvgs']) > 5:
        print(f"[FAIL] Active FVG list exceeds maximum limit of 5. Found: {len(fvg_data['fvgs'])}")
        sys.exit(1)
    else:
        print(f"[PASS] Active FVG list is sliced to max 5 items.")
    ...
    if fvg_data['nearest_fvg_high'] is not None:
        expected_dist = round(((ltp - fvg_data['nearest_fvg_high']) / fvg_data['nearest_fvg_high']) * 100, 2)
        if abs(expected_dist - fvg_data['nearest_fvg_dist_pct']) > 0.01:
            print(f"[FAIL] Nearest FVG distance calculation mismatch. Expected: {expected_dist}%, Got: {fvg_data['nearest_fvg_dist_pct']}%")
            sys.exit(1)
        else:
            print(f"[PASS] Distance to nearest active FVG matches expected formula exactly.")
