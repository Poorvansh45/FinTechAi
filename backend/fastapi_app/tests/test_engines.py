...
from engines.indicators import IndicatorEngine, ema, rsi
from engines.patterns import classify_continuation, detect_bullish_fvgs
    ...
    df = pd.DataFrame(
        {
            ...
            "Open": [98, 101, 104, 106],
            "High": [100, 106, 108, 109],
            "Low": [97, 101, 103, 105],
            "Close": [99, 105, 107, 106],
            "Volume": [1, 1, 1, 1],
        }
    )
...
def test_classify_continuation_states():
    ...
    assert (
        classify_continuation(f, 104, max_distance_pct=3.0) == "at_support"
    )  # 0.97% above
    assert (
        classify_continuation(f, 110, max_distance_pct=3.0) == "extended"
    )  # 6.8% above
...
def test_tradeplan_from_support_uses_atr_and_reward_multiple():
    tp = TradePlan.from_support(
        entry=100.0, support_level=96.0, atr=2.0, reward_multiple=2.0
    )
...
def test_tradeplan_widens_implausibly_tight_stop():
    ...
    tp = TradePlan.from_support(
        entry=100.0, support_level=99.9, atr=2.0, reward_multiple=2.0
    )
...
def test_tradeplan_atr_fallback_when_missing():
    tp = TradePlan.from_support(
        entry=100.0, support_level=96.0, atr=None, reward_multiple=2.0
    )
    assert (
        tp.stop_loss < 96.0 and tp.risk_reward == 2.0
    )  # uses 2% fallback, still valid
...
def test_alphazone_in_zone_uses_cmp_entry():
    r = build_alphazone_result(
        symbol="X", company_name="X", ltp=97.0, ob=_ob(), ema200_dist_pct=5.0, atr=2.0
    )
...
def test_alphazone_confidence_rewards_fresh_close_zones():
    fresh, bd = score_confidence(
        institutional_score=80, distance_pct=0.0, touch_count=0, ema200_dist_pct=5.0
    )
    retested_far, _ = score_confidence(
        institutional_score=80, distance_pct=2.5, touch_count=2, ema200_dist_pct=5.0
    )
...
def test_alphazone_ob_close_below_floor_invalidates_but_wick_survives():
    ...
    wick = detect_internal_bullish_obs(
        _append(df, close=floor + 3, low=floor - 2, high=floor + 4)
    )
    assert any(abs(o.zone_low - floor) < 1e-6 for o in wick)

    # A candle that CLOSES below the floor → that zone is invalidated (gone).
    closed = detect_internal_bullish_obs(
        _append(df, close=floor - 2, low=floor - 3, high=floor + 1)
    )
