...
async def main():
    ...
    symbols = [
        ...
        "SBIN", "BHARTIARTL", "BAJFINANCE", "KOTAKBANK", "HINDUNILVR", 
        "AXISBANK", "LT", "ASIANPAINT", "MARUTI", "SUNPHARMA", 
        "TITAN", "ULTRACEMCO", "TATAMOTORS", "ZOMATO", "SUZLON", "PAYTM"
    ]
    
    print(f"Populating SMC data for {len(symbols)} popular symbols using local MongoDB OHLC cache...")
    for symbol in symbols:
        try:
            ...
            if len(docs) < 50:
                print(f"Skipping {symbol}: not enough historical data in DB ({len(docs)} candles)")
                continue
            ...
            df = pd.DataFrame([{
                ...
                "Close": d.get("close"),
                "Volume": d.get("volume"),
            } for d in docs]).dropna(subset=["Close", "High", "Low"])
