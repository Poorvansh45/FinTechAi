...
def run_migration():
    ...
    for coll_name in collections_to_dedup:
        ...
        for norm_symbol, symbol_docs in groups.items():
            if len(symbol_docs) > 1:
                ...
                _latest_original_symbol, latest_doc = sorted_docs[0]
                ...
                ops.append(
                    UpdateOne(
                        {"_id": latest_doc["_id"]}, {"$set": {"symbol": norm_symbol}}
                    )
                )
            else:
                ...
                if orig_symbol != norm_symbol:
                    ops.append(
                        UpdateOne({"_id": d["_id"]}, {"$set": {"symbol": norm_symbol}})
                    )
        ...
        if ops:
            log.info(
                f"Executing bulk write of {len(ops)} operations on collection: {coll_name}"
            )
    ...
    print(
        f"4. Total symbols scanned:          {meta_doc.get('total_symbols') if meta_doc else screener_cache_count}"
    )
    print(
        f"5. Metadata record count:          {meta_doc.get('record_count') if meta_doc else screener_cache_count}"
    )
    print(f"6. Table record count:             {screener_cache_count}")
    print(
        f"7. Status bar record count:        {meta_doc.get('symbols_processed') if meta_doc else screener_cache_count}"
    )
    print(f"8. Matched badge count:            {screener_cache_count}")

    counts_match = (
        screener_cache_count
        == meta_doc.get("record_count")
        == meta_doc.get("symbols_processed")
    )
    print(
        f"Counts Match Verification:         {'✓ MATCHED' if counts_match else '✗ MISMATCHED'}"
    )
    print("==========================================\n")
