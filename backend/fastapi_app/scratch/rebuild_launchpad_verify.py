...
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # fastapi_app on path
...
async def run():
    ...
    print(f"Mongo host: {client.address if hasattr(client,'address') else uri.split('@')[-1][:40]}")
    print(f"Database  : finai_edge\n")
    ...
    if old:
        ...
        print("  OLD breakdown keys:", sorted((s.get("confidence_breakdown") or {}).keys()))
    print()
    ...
    if new:
        ...
        print("  NEW breakdown keys (should have no 'rsi'):", sorted((s.get("confidence_breakdown") or {}).keys()))
    print()
    ...
    for sym in added[:60]:
        d = new[sym]
        print(f"  + {sym:14} cmp={d.get('cmp'):>8}  conf={d.get('confidence'):>5}  setup={d.get('setup')}")
    if len(added) > 60:
    ...
    for sym in removed[:60]:
        ...
        print(f"  - {sym:14} cmp={d.get('cmp'):>8}  oldconf={d.get('confidence'):>5}  [{reason}]")
    if len(removed) > 60:
