...
from scanners.fvg import detect_bullish_fvgs, analyze_fvg_for_symbol
...
async def audit():
    ...
    if fvgs:
        latest = fvgs[-1]
        print(f"   Latest FVG Date: {latest['end_date']}, Low: {latest['low']}, High: {latest['high']}, Gap %: {latest['gap_pct']}%")
    ...
    if os.path.exists(stock_fvg_path):
        ...
        print(f"3. Stock_FVG.csv: {len(bluechip_fvg_csv)} rows found for BLUECHIP (due to RSI/other filters).")
    else:
    ...
    if doc:
        print(f"4. Database Storage: Document found.")
        print(f"   fvg_count field: {doc.get('fvg_count')}")
        print(f"   fvgs array size: {len(doc.get('fvgs', []))}")
        if doc.get('fvgs'):
            lat = doc.get('fvgs')[0]
            print(f"   Latest cached FVG: {lat.get('end_date')} Low: {lat.get('low')} High: {lat.get('high')} Gap: {lat.get('gap_pct')}%")
    else:
    ...
    print(f"{'Symbol':15} | {'Expected (Ref)':15} | {'Actual (New)':15} | {'Difference':10}")
    print("-" * 60)
    diff_count = 0
    ...
    for sym in ['BLUECHIP'] + sample_syms[:4]:
        ...
        print(f"Symbol: {sym:12} | Total: {len(sym_fvgs):4} | Active (Unmitigated: Low > FVG High): {active_cnt_high:4} | Active (Not fully filled: Low > FVG Low): {active_cnt_low:4}")
