...
async def run():
    ...
    fvg_bullish_count = await db.screener_cache.count_documents({"fvg.has_fvg_bullish": True})
    print(f"  fvg.has_fvg_bullish == True: {fvg_bullish_count}")
    
    fvg_bearish_count = await db.screener_cache.count_documents({"fvg.has_fvg_bearish": True})
    print(f"  fvg.has_fvg_bearish == True: {fvg_bearish_count}")
    
    has_fvg_field = await db.screener_cache.count_documents({"f": {"$ne": None}}) # wait, key is fvg
    print(f"  fvg field exists: {await db.screener_cache.count_documents({'fvg': {'$ne': None}})}")
    ...
    if fvg_bullish_count > 0:
        ...
        print(f"  Sample fvg.has_fvg_bullish document: {doc['symbol']}, fvg: {doc['fvg']}")
    ...
    active_fvgs_gt_0 = await db.fvg_scan_results.count_documents({"active_fvg_count": {"$gt": 0}})
    print(f"  active_fvg_count > 0: {active_fvgs_gt_0}")
    
    nearest_fvg_not_null = await db.fvg_scan_results.count_documents({"nearest_fvg_dist_pct": {"$ne": None}})
    ...
    if active_fvgs_gt_0 > 0:
        ...
        print(f"  Sample active FVG: {doc['symbol']}, active_fvg_count: {doc['active_fvg_count']}, nearest_dist: {doc['nearest_fvg_dist_pct']}")
