...
from services.watchlist_service import WatchlistService, get_watchlist_service
...
async def create_watchlist(
    data: WatchlistCreate, request: Request, user_id: str = Depends(get_current_user)
):
...
async def get_watchlist_leaderboard(
...
async def get_watchlist_details(
    id: str, request: Request, user_id: str = Depends(get_current_user)
):
    svc = get_service(request)
...
async def remove_stock(
    id: str, symbol: str, request: Request, user_id: str = Depends(get_current_user)
):
    svc = get_service(request)
    await svc.remove_stock(id, symbol, user_id)
    return {
        "success": True,
        "message": f"Stock {symbol} removed and archived to history",
    }
