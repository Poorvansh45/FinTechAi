...
async def get_smc_results(
    ...
    min_score: int | None = Query(None, description="Minimum SMC Score 0-100"),
    event: str | None = Query(None, description="BOS|CHoCH"),
...
async def get_zone_proximity(
    request: Request,
    distance_pct_max: float = Query(
        10.0, description="Max distance % from zone edge (default 10%)"
    ),
    limit: int = Query(200),
):
    """
...
async def trigger_scan(request: Request, symbol: str):
    ...
    return {
        "success": True,
        "message": f"Scan completed for {symbol.upper()}",
        "data": result,
    }
