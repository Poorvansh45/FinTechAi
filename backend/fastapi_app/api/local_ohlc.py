...
def _market(request: Request):
    if (
        not hasattr(request.app.state, "market_service")
        or request.app.state.market_service is None
    ):
...
async def get_local_ohlc_status(request: Request):
    ...
    for file_name in os.listdir(data_dir):
        if file_name.lower().endswith(".csv"):
            ...
            screener_exists = (
                await db.screener_cache.find_one({"symbol": symbol, "is_local": True})
                is not None
            )
            fvg_exists = (
                await db.fvg_cache.find_one({"symbol": symbol, "is_local": True})
                is not None
            )
            smc_exists = (
                await db.smc_scanner_results.find_one(
        ...
        combined[sym].update(
            {
                ...
                "fvg_status": doc.get("top_bullish_fvgs", [{}])[0].get("status", "N/A")
                if doc.get("top_bullish_fvgs")
                else "N/A",
                "fvg_strength": doc.get("top_bullish_fvgs", [{}])[0].get(
                    "strength", "N/A"
                )
                if doc.get("top_bullish_fvgs")
                else "N/A",
...
async def trigger_local_ohlc_scan(request: Request, background_tasks: BackgroundTasks):
    ...
    return {
        "success": True,
        "message": "Incremental sync & scan for local OHLC files triggered in the background.",
    }
...
async def upload_local_ohlc(request: Request, file: UploadFile = File(...)):
    ...
    if not required.issubset(headers) and not {
        "timestamp",
        "open",
        "high",
        "low",
    ...
    if not scan_res:
        ...
        try:
            ...
            log.debug(f"cleanup of {resolved_path} failed: {e}")
        raise HTTPException(
            status_code=400,
            detail="Failed to process stock OHLC candles. Verify data formatting.",
        )

    return {
        "success": True,
        "message": f"Successfully uploaded and scanned {clean_name}",
        "data": scan_res,
    }
